...
- **Navigation** — `Sales channels` and `Apps` are real `Navigation.Section`
  ...
  whole section.
- **Page header** — one shared `components/shell/page-header.tsx` renders the
  breadcrumb (area icon, `›`, title, right-aligned actions, optional subtitle and
  title badges). It is now on **every** admin page, index and detail: the
  product form's local `Breadcrumb` was deleted in favour of it, and every
  remaining Polaris `backAction` is gone. That closes the follow-up
  [product-form.md](product-form.md#delta-vs-our-build) logged.
- **Footer line** — `components/shell/learn-more.tsx` on Orders, Products,
  Collections, Customers, Discounts, Inventory and Apps. The hrefs are real
  `help.shopify.com` pages, because a link that goes nowhere is the dead control
