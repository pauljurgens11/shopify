...
- **Skills.** `.claude/skills/` ships three workflows every agent gets automatically. They split by *what you are doing*, not by topic: **`resolve-issue`** — build one issue from `docs/issues/` into a merged PR. **`critical-review`** — audit ONE named scope (workstream, module, PR) in depth; reports, never fixes. **`repo-review`** — survey the WHOLE build: drive the running app, verify what the swarm *claims* is done, find what broke between workstreams, land the small fixes. Invoke by name, or just describe the task and let it trigger.
